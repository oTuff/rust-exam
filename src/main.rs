...
use task_manager::{load_from_file, save_to_file, TaskManager, Task, Status};
...
fn main() {
    ...
    match matches.subcommand() {
        Some(("add", sub_m)) => {
            ...
            let priority = sub_m.get_one::<String>("priority").unwrap().parse::<u8>().unwrap();
            ...
            task_manager.add_task(title.clone(), description.clone(), priority, Status::Todo, deadline);
            save_to_file(&task_manager, file_path).expect("Failed to save tasks");
        },
        Some(("delete", sub_m)) => {
            let id = sub_m.get_one::<String>("id").unwrap().parse::<usize>().unwrap();
        ...
        Some(("list", _)) => {
            for task in task_manager.list_tasks() {
                println!("ID: {}, Title: {}, Status: {:?}, Description: {}", task.id, task.title, task.status,task.description);
            }
        },
...
fn parse_date_string(date_str: &str) -> Result<DateTime<Utc>, ParseError> {
    ...
    Ok(Local.from_local_date(&local_date).unwrap().and_hms(0, 0, 0).with_timezone(&Utc))
}
